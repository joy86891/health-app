 ...
 *
 * ## 5. 固定底部输入框 (forceKeyboardAvoid)
 * - 场景：页面有固定在底部的输入框（如聊天页面）。
 * - 用法：`<Screen forceKeyboardAvoid={true}>`
 * - 行为：强制使用全局键盘避让，整个页面（包括固定底部栏）都会响应键盘。
 */
interface ScreenProps {
  ...
  /**
   * 强制启用全局键盘避让
   * - 默认: false (当检测到滚动组件时，只对滚动组件进行键盘避让)
   * - 设为 true: 强制使用全局键盘避让，适用于固定底部输入框的场景
   */
  forceKeyboardAvoid?: boolean;
}
...
const KeyboardAwareScrollable = ({
  ...
  const commonProps = {
    ...
    // 如果子组件已设置 keyboardShouldPersistTaps，使用子组件的设置，否则使用 'handled'
    keyboardShouldPersistTaps: childAttrs['keyboardShouldPersistTaps'] ?? 'handled',
    // 如果子组件已设置 keyboardDismissMode，使用子组件的设置，否则使用 'none'（不自动关闭键盘）
    keyboardDismissMode: childAttrs['keyboardDismissMode'] ?? 'none',
...
export const Screen = ({
  ...
  const wrapperStyle: ViewStyle = {
    ...
    // Web 端不响应键盘状态，避免布局抖动
    paddingBottom: (childIsNativeScrollable && hasBottom && !forceKeyboardAvoid)
      ? (keyboardShown && Platform.OS !== 'web' ? 0 : insets.bottom)
      : 0,
  };

  // 若子树不可滚动，或强制启用全局键盘避让，则外层使用 KeyboardAwareScrollView 提供"全局页面滑动"能力
  // forceKeyboardAvoid 用于固定底部输入框场景，需要整个页面响应键盘
  const useScrollContainer = !childIsNativeScrollable || forceKeyboardAvoid;
  ...
  // 注意：当 forceKeyboardAvoid 为 true 时，不会执行此逻辑，因为已经使用全局键盘避让
